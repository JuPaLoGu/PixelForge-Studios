...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($nombre) || empty($apellido) || empty($correo) || !filter_var($correo, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($resultado->num_rows === 0) {
            ...
            if ($res_inscrito->num_rows > 0) {
                ...
                $stmt = $conexion->prepare("SELECT cupos_disponibles FROM cursos WHERE id = ?");
                ...
                if ($resultado_curso->num_rows > 0) {
                    ...
                    if ($curso["cupos_disponibles"] > 0) {
                        ...
                        if ($stmt->execute()) {
                            $conexion->query("UPDATE cursos SET cupos_disponibles = cupos_disponibles - 1 WHERE id = $curso_id");
  ...
  <div class="form-container">
    <form action="formulario_inscripcion.php" method="POST">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          <label for="apellido"><i class="fa fa-user"></i> Last Name</label>
